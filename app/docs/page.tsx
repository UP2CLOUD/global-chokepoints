          ...
          <div id="history" className="scroll-mt-20">
            <EndpointCard
              ...
              params={[
                ...
                { name: 'since', in: 'query', type: 'ISO 8601', desc: 'Return only transitions recorded after this timestamp' },
...
curl "${SITE}/v1/history?state=CLOSED&limit=10"`}
              jsExample={`const { items } = await fetch('${SITE}/v1/history?limit=50').then(r => r.json());
items.forEach(({ state, previousState, tension, timestamp }) => {
  console.log(\`\${timestamp}: \${previousState} → \${state} (tension \${tension})\`);
});`}
            />
          </div>
