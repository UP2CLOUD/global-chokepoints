...
function validDate(d: string): Date | null {
  if (!d) return null;
  const dt = new Date(d);
...
export function fmtDate(d: string, locale: string = 'en-US') {
  ...
  return dt ? dt.toLocaleDateString(locale, { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : '—';
}

export function fmtDateShort(d: string, locale: string = 'en-US') {
  const dt = validDate(d);
  return dt ? dt.toLocaleDateString(locale, { day: '2-digit', month: 'short' }) : '—';
}

export function fmtTime(d: string, locale: string = 'en-US') {
  const dt = validDate(d);
  return dt ? dt.toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit' }) : '—';
}

export function timeAgo(dateStr: string, nowStr = 'now', hAgo = 'h ago', dAgo = 'd ago'): string {
  const dt = validDate(dateStr);
  if (!dt) return nowStr;
